...
class MjViewerBasic(cymj.MjRenderContextWindow):
    ...
    def __init__(self, sim):
        ...
        #debug
        # glfw.set_key_callback(self.window, self.key2_callback)
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        #  debug
        # if self._shift:
        #     if self.sim.model and self.pert.select>0:
        #         self.pert.select=sim.model.body_parentid[self.pert.select]
    ...
    def _cursor_pos_callback(self, window, xpos, ypos):
        ...
        #     debug
        with self._gui_lock:
            # print('self.pert.active', self.pert.active)
            if self.pert.active:
    ...
    # def _cursor2_callback(self, window, xpos, ypos):
    #     self.x = int(xpos* self._scale)
    #     self.y = int(ypos* self._scale)
...
class MjViewer(MjViewerBasic):
    ...
    def _create_full_overlay(self):
        ...
        # debug
        # self.add_overlay(const.GRID_BOTTOMLEFT, "x/y", '%d/%d' % (self.x, self.y))
        ...
        # self.add_overlay(const.GRID_TOPLEFT, "Apply [CONTROL] force", "On" if self._ctrl else "Off")
        # self.add_overlay(const.GRID_TOPLEFT, "Apply [SHIFT] choose", "On" if self._shift else "Off")

    def key_callback(self, window, key, scancode, action, mods):
        # the key has been pressed then released, enter logic
        ...
        # debug
            ...
            vopt = self.vopt
            vopt.flags[8] = not vopt.flags[8]
        elif key == glfw.KEY_O:
            vopt = self.vopt
            vopt.flags[9]= not vopt.flags[9]

        # if not self._ctrl:
        #     self.pert.active=0
        # call parent's class at the end, otherwise will be shadowed
