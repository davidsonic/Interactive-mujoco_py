...
cdef class MjRenderContext(object):
    ...
    def render(self, width, height, camera_id=None):
        ...
        if hasattr(self, 'window'):
            swidth, sheight=glfw.get_framebuffer_size(self.window)
            if self._needselect:
                # print('enter needselect', self._needselect)
                selgeom = mjv_select(self._model_ptr, self._data_ptr, &self._vopt,
                                     width/height,
                                     (self._last_mouse_x)/width, (height -self._last_mouse_y)/height,
                                     &self._scn, &selpnt[0])
                # print('selgeom', selgeom)

                if self._needselect ==2:
                    if selgeom >=0:
                        mju_copy3(self._cam.lookat, &selpnt[0])
                    if selgeom >0:
                        self.cam.type= const.CAMERA_TRACKING
                        self.cam.trackbodyid = selgeom
                        self.cam.fixedcamid = -1
                # left button double click
                else:
                    if selgeom>=0:
                        # print('self._scn.ngeom', self._scn.ngeom)
                        g = <mjvGeom*>(self._scn.geoms + selgeom)
                        # print('g[0].objid', g[0].objid)
                        selbody = self.sim.model.geom_bodyid[g[0].objid]
                        self.pert.select= selbody
                        vec2= <mjtNum[:3]> (self._data_ptr.xpos + 3* self._pert.select)
                        mju_sub3(&tmp[0], &selpnt[0], &vec2[0])
                        mju_mulMatTVec(self._pert.localpos, (self._data_ptr.xmat + 9* self._pert.select), &tmp[0], 3,3)
                    else:
                        selbody=0
                        self.pert.select=selbody
    ...
    # debug
    def move_perturb(self, action, dx, dy):
        mjv_movePerturb(self._model_ptr, self._data_ptr, action, dx, dy, &self._scn, &self._pert)

    def apply_perturbPose(self, int flg_paused):
        mjv_applyPerturbPose(self._model_ptr, self._data_ptr, &self._pert, flg_paused)
    #
    def init_perturbforce(self):
        mjv_initPerturb(self._model_ptr, self._data_ptr, &self._scn, &self._pert)
    #
    def apply_perturbforce(self):
        mjv_applyPerturbForce(self._model_ptr, self._data_ptr, &self._pert)
